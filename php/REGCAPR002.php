...
if(login_check($mysql_con) == true){
	...
	$stmt = $mysql_con->prepare("SELECT num_tit,num_par,val_tit,raz_social,dat_venc FROM titulos AS tit,  fornecedores as forn where tit.ID_Forn = forn.id_forn AND dat_baix IS NULL ORDER BY dat_venc");
	$stmt->execute();
	$stmt->bind_result($num_tit,$num_par,$val_tit,$raz_social,$dat_venc);
	
	$html = '';
	
	$html = $html . getHeaderRel('Relatório Contas a Pagar');//'<h1>RelatÃ³rio Contas a Pagar</h1>';
