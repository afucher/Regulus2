...
//if($totalrows) {$limit = $totalrows;}
	...
	$stmt = $mysql_con->prepare("SELECT num_tit,num_par,val_tit,raz_social FROM titulos AS tit,  fornecedores as forn where tit.ID_Forn = forn.id_forn AND dat_baix IS NULL");
	$stmt->execute();
	$stmt->bind_result($num_tit,$num_par,$val_tit,$raz_social);
	while($stmt->fetch()){
