	...
	<link rel="stylesheet" href="css\menu.css">
		...
		<script type="text/javascript">
			jQuery(document).ready(function(){
				jQuery("#browse").jqGrid({
					...
					url:'adoCAP.php',
					datatype: "json",
					colNames:['No. Título','Parcela','Fornecedor','Valor'],
					colModel:[
		...
		$(function() {
			...
			buttons: {
				"Baixar": function() {
				  $( this ).dialog( "close" );
				  var param = {"dt_baixa":$("#dt_baixa").val(),"val_pago":$("#val_pago").val(),"titulo":getSelTitulo(),"parcela":getSelParcela()};
...
<div id="dialog_baixa" title="Baixar título..." style="width:auto !important;">
	<form id="form_baixa" action="php/baixaCAP.php">
		<label>Data de Baixa: 
			<input id="dt_baixa" type="date"/>
		</label>
		<label>Valor pago: 
			<input id="val_pago" type="text"/>
		</label>
...
function getSelTitulo(){
	...
	if (colSel==null)
	{
		...
		return myGrid.jqGrid('getCell',colSel,'num_tit');
	}	
}
...
function getSelParcela(){
	...
	if(colSel==null){
		...
		return myGrid.jqGrid('getCell',colSel,'num_par');
	}
}
//-------------------------------------
...
function getSelValTit(){
	...
	if(colSel==null){
		...
		return myGrid.jqGrid('getCell',colSel,'val_tit');
	}
}
